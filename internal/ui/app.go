...
func (m Model) View() string {
	...
	case m.cfg.VimMode && m.mode == modeNormal:
		b.WriteString(lipgloss.NewStyle().Foreground(colBlue).Bold(true).Render(" NORMAL ") +
			styleHints.Render(" i//: search  j/k gg/G ^d/^u: move  enter/l: open  h: collapse  tab: expand  ^w/^g/^f/^x: worktree/clone/fetch/delete  q: quit"))
	case m.cfg.VimMode:
	...
	default:
		b.WriteString(styleHints.Render("enter: open  tab: expand worktrees  ctrl+w: new worktree  ctrl+g: clone repo  ctrl+f: fetch  ctrl+x: delete worktree  ↑↓: navigate  esc/ctrl+c: quit"))
	}
