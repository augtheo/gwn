...
// assistantCmd adjusts the configured assistant command for the "agent"
...
func OpenWorkspace(session, dir, editor, assistant, diffCmd string) error {
	cmd, err := PrepareOpen(session, dir, editor, assistant, diffCmd)
	if err != nil {
		return err
	}
	if cmd == nil {
...
//
// diffCmd is the command run in the "diff" window — cfg.ReviewCommand for a
// PR worktree, cfg.DiffCommand otherwise.
//
...
func PrepareOpen(session, dir, editor, assistant, diffCmd string) (*exec.Cmd, error) {
	...
	if err := RenameCurrentWindow(session, "lazygit"); err != nil {
		return nil, err
	}
	// lazygit doesn't set the terminal title, but disable automatic-rename
	// anyway so tmux never overwrites the "lazygit" window name.
	if err := DisableAutomaticRename(session, "lazygit"); err != nil {
		return nil, err
	}
	if err := NewWindow(session, "editor", dir); err != nil {
		return nil, err
	}
	if err := NewWindow(session, "agent", dir); err != nil {
		return nil, err
	}
	// Claude Code sets the terminal title to a chat summary; without this,
	// tmux's automatic-rename would overwrite the "agent" window name with it.
	if err := DisableAutomaticRename(session, "agent"); err != nil {
		return nil, err
	}
	if err := NewWindow(session, "diff", dir); err != nil {
		return nil, err
	}
	if err := NewWindow(session, "shell", dir); err != nil {
		return nil, err
	}
	if err := SelectWindow(session, "lazygit"); err != nil {
		return nil, err
	}
	...
	if InsideTmux() {
		...
		if err := SendKeysToWindow(session, "lazygit", "lazygit"); err != nil {
			return nil, err
		}
		if err := SendKeysToWindow(session, "editor", editor); err != nil {
			return nil, err
		}
		if err := SendKeysToWindow(session, "agent", assistant); err != nil {
			return nil, err
		}
		return nil, SendKeysToWindow(session, "diff", diffCmd)
	}
	...
	if err := SendKeysToWindow(session, "diff", diffCmd); err != nil {
		return nil, err
	}
	return AttachSessionCmd(session), nil
}
